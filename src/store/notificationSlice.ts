...
export const fetchNotifications = createAsyncThunk(
  "notifications/fetch",
  async (
    params: { page: number; perPage: number; filters?: { is_read?: number } },
    { rejectWithValue }
  ) => {
    try {
...
const notificationSlice = createSlice({
  ...
  reducers: {
    ...
    updateNotification: (
      state,
      action: PayloadAction<Partial<Notification> & { id: string }>
    ) => {
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(fetchNotifications.fulfilled, (state, action) => {
        ...
        if (action.payload.page === 1) {
          // Merge new unread notifications with existing read notifications
          // This preserves read notifications that were marked as read in the UI
          const existingReadNotifications = state.notifications.filter(
            (n) => n.is_read
          );
          const newUnreadNotifications = action.payload.data.filter(
            (newItem: Notification) => !newItem.is_read
          );

          // Combine: new unread + existing read (avoid duplicates)
          // Keep existing read notifications that are not in the new unread list
          const preservedReadNotifications = existingReadNotifications.filter(
            (existing: Notification) =>
              !newUnreadNotifications.some(
                (newItem: Notification) => newItem.id === existing.id
              )
          );

          // Merge: new unread notifications first, then preserved read notifications
          const allNotifications = [
            ...newUnreadNotifications,
            ...preservedReadNotifications,
          ];
    ...
    builder.addCase(markNotificationAsRead.fulfilled, (state, action) => {
      ...
      if (notification && !notification.is_read) {
        ...
        // Keep notification in the list - don't remove it
        // This allows users to see their read notifications even after marking as read
      }
    });
...
export const selectIsMarkingAllRead = (state: RootState) =>
  state.notifications.isMarkingAllRead;
