...
const AlbumDropdown: React.FC<AlbumDropdownProps> = ({
  ...
  const [originalClickPosition, setOriginalClickPosition] = useState<{
    x: number;
    y: number;
  } | null>(null);
  ...
  const { data: sharedAlbumsResponse, isLoading: sharedAlbumsLoading } =
    getAlbumMemberList(mediaId, showDropdown);
  ...
  const componentId =
    providedId ??
    `album-dropdown-${mediaId}-${Math.random().toString(36).substr(2, 9)}`;
  ...
  const currentAlbums =
    activeTab === ALBUM_TABS.MY_ALBUMS
      ? filteredMyAlbums
      : filteredSharedAlbums;
  const currentLoading =
    activeTab === ALBUM_TABS.MY_ALBUMS ? myAlbumsLoading : sharedAlbumsLoading;
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (showDropdown) {
        ...
        const isInsideDropdown = Array.from(dropdownMenuElements).some((menu) =>
          menu.contains(target)
        );

        // Check if click is inside any modal (to prevent closing when modal opens)
        const isInsideModal =
          target.closest(".ant-modal") || target.closest('[role="dialog"]');
        ...
        if (triggerElement) {
          const newPosition = calculateDropdownPosition(
            ...
            position === "click" && originalClickPosition
              ? ({
                  pageX: originalClickPosition.x,
                  pageY: originalClickPosition.y,
                } as React.MouseEvent)
              : undefined
          );
          setDropdownPosition(newPosition);
        ...
        createPortal(
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto custom-scroll-bar">
              <Loading isLoading={currentLoading}>
                  ...
                  <Empty
                    ...
                    description={
                      searchTerm
                        ...
                        : activeTab === ALBUM_TABS.MY_ALBUMS
                        ? "Create an album first to save media"
