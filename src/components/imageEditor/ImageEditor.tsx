...
const ImageEditorComponent: React.FC<ImageEditorProps> = ({
  ...
  useEffect(() => {
    ...
    if (isVisible && editorRef.current && !tuiEditorRef.current) {
      ...
      setTimeout(() => {
        const downloadBtn = document.querySelector('.tui-image-editor-header-buttons .tui-image-editor-download-btn');
        if (downloadBtn) {
          (downloadBtn as HTMLElement).style.display = 'none';
        }
        
        // Also try alternative selectors
        const downloadBtns = document.querySelectorAll('[title="Download"], .tie-btn-download, .tui-image-editor-download');
