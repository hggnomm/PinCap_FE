...
const ReportUsersView: React.FC = () => {
  ...
  const [searchParams, setSearchParams] = useState<GetUserReportsParams>({
    page: 1,
    per_page: 10,
  });
  const [isEditModalVisible, setIsEditModalVisible] = useState(false);
  const [editingReport, setEditingReport] = useState<AdminUserReport | null>(
    null
  );
  ...
  const fetchReports = async (calculateTotal = false) => {
    setLoading(true);
    try {
      const response = await getUserReports(searchParams);
      ...
      if (response.data.length === 0 && pagination.total === 0) {
        ...
        try {
          const countResponse = await getUserReports({
            ...searchParams,
            page: 1,
            per_page: 1000,
          });
      ...
      const errorMessage =
        error && typeof error === "object" && "message" in error
          ...
          : "Failed to fetch user reports. Please try again.";
  ...
  const handleSearchReporterId = (value: string) => {
    setSearchParams((prev) => ({
      ...prev,
      user_report_id: value || undefined,
      page: 1,
    }));
    setPagination((prev) => ({ ...prev, current: 1 }));
  };
  ...
  const handleClearFilters = () => {
    const defaultParams: GetUserReportsParams = {
      page: 1,
      per_page: 10,
    };
  ...
  const handleEdit = (report: AdminUserReport) => {
  ...
  const handleEditSubmit = async (values: EditReportFormData) => {
    ...
    try {
      const updateData: UpdateUserReportData = {
        report_state: values.report_state,
        reason_report_id: values.reason_report_id || null,
        other_reasons: values.other_reasons || null,
      };

      await updateUserReport(editingReport.id, updateData);
      message.success("User report updated successfully");
  ...
  const handleDelete = async (reportId: string) => {
    try {
      ...
      message.success("User report deleted successfully");
      fetchReports();
    } catch (error: unknown) {
      const errorMessage =
        error && typeof error === "object" && "message" in error
  ...
  const handleRestore = async (reportId: string) => {
    try {
      await restoreUserReport(reportId);
      message.success("User report restored successfully");
      fetchReports();
    } catch (error: unknown) {
      const errorMessage =
        error && typeof error === "object" && "message" in error
          ? String(error.message)
          : "Failed to restore user report";
      message.error(errorMessage);
    }
  };
  ...
  const handleForceDelete = async (reportId: string) => {
    try {
      await forceDeleteUserReport(reportId);
      message.success("User report permanently deleted successfully");
      fetchReports();
    } catch (error: unknown) {
      const errorMessage =
        error && typeof error === "object" && "message" in error
          ? String(error.message)
          : "Failed to permanently delete user report";
      message.error(errorMessage);
    }
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_: unknown, record: AdminUserReport) => {
        if (record.reporter) {
          return `${record.reporter.first_name} ${record.reporter.last_name}`;
        }
        return record.user_report_id || "-";
      },
    },
    {
      title: "Reason",
        ...
        <Space direction="vertical" size="middle" style={{ width: "100%" }}>
          <Space>
            <Input
              ...
              onPressEnter={(e) => handleSearchUserId(e.currentTarget.value)}
              onChange={(e) => {
                if (!e.target.value) {
                  handleSearchUserId("");
                }
            ...
            <Input
              ...
              allowClear
              style={{ width: 200 }}
              value={searchParams.user_report_id || ""}
              onPressEnter={(e) =>
